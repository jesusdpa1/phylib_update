    ...
    if mmap_mode is None:
        for w in ('nan', 'inf'):
            ...
            if np.any(errors):
                ...
                n_tot = errors.size
                logger.warning(
                    f'{n}/{n_tot} values are {w} in {path}, replacing by zero.'
                )
                out[errors] = 0
    return out
...
class TemplateModel:
    ...
    def _load_data(self):
        ...
        if self.spike_times[-1] > self.duration:  # pragma: no cover
            logger.warning(
                f'There are {np.sum(self.spike_times > self.duration)}/'
                f'{self.n_spikes} spikes after the end of the recording.'
            )
