    ...
    if mmap_mode is None:
        for w in ('nan', 'inf'):
            ...
            if np.any(errors):
                n = np.sum(errors)
                n_tot = errors.size
                logger.warning(f'{n}/{n_tot} values are {w} in {path}, replacing by zero.')
                out[errors] = 0
    return out
...
def _close_memmap(name, obj):
    ...
    if isinstance(obj, np.memmap):
        logger.debug(f'Close memmap array {name}.')
        obj._mmap.close()
...
class TemplateModel:
    ...
    def _load_data(self):
        ...
        if self.spike_times[-1] > self.duration:  # pragma: no cover
            logger.warning(
                f'There are {np.sum(self.spike_times > self.duration)}/{self.n_spikes} spikes after the end of the recording.'
            )
    ...
    def _load_metadata(self):
        ...
        for filename in files:
            if filename.stem in excluded_names:
                continue
            logger.debug(f'Load `{filename.name}`.')
            try:
                for field, data in load_metadata(filename).items():
                    metadata[field] = data
            except Exception as e:
                logger.warning(f'Error when reading {filename.name}: {str(e)}')
                continue
        return metadata
    ...
    def _load_spike_attributes(self):
        ...
        for filename in files:
            ...
            try:
                ...
                logger.warning(f'Unable to open {filename.name}: {e}')
                continue
    ...
    def _load_spike_waveforms(self):  # pragma: no cover
        ...
        try:
            ...
            logger.warning(f'Could not load spike waveforms: {e}')
            return
    ...
    def _compute_wmi(self, wm):
        logger.debug(f'Inversing the whitening matrix {wm.shape}.')
        try:
    ...
    def save_spike_clusters(self, spike_clusters):
        ...
        logger.debug(f'Save spike clusters to `{path}`.')
    ...
    def save_spikes_subset_waveforms(
        ...
        # Save the spike ids.
        ns = len(spike_ids)
        logger.debug(f'Saving spike waveforms: {ns} spikes.')
